...
export default function Admin() {
  ...
  const manualClaim = async (mission: any) => {
    const requestData = {
      method: "POST",
      header: {
        "Content-Type": "application/json",
    ...
    var response = await fetch("./api/adventures/manualClaim", requestData);
  ...
  const RenderMissions = () => {
    const AddMission = () => {
      ...
      const [ttccOnly, setTTCCOnly]: any = useState(false);
      const [repeat, setRepeat]: any = useState(false);
        ...
        <div className="indivMission addAMission">
          <div className="missionInfo">
            ...
            <h1 style={{ fontSize: "14px", minWidth: "100%" }}>
              ...
              />
              Allow user to repeat mission?
            </h1>
            <h1 style={{ fontSize: "14px", minWidth: "100%" }}>
              <input
                type="checkbox"
                checked={ttccOnly}
                onChange={() => setTTCCOnly(!ttccOnly)}
              />
              TTCC Chimps only?
            </h1>
            ...
            <div key={`missions ${index}`} className="indivMission">
              ...
              <h1 style={{ fontSize: "14px", minWidth: "100%" }}>
                <input
                  style={{ margin: "0 10px" }}
                  type="checkbox"
                  checked={mission.data.repeat}
                  onChange={async () => {
                    await writeAPI("updateRepeat", null, {
                      id: mission.id,
                      value: !mission.data.repeat,
                    });
                    setRefresh(!refresh);
                  }}
                />
              ...
              <h1 style={{ fontSize: "14px", minWidth: "100%" }}>
                <input
                  style={{ margin: "0 10px" }}
                  type="checkbox"
                  checked={mission.data.ttccOnly}
                  onChange={async () => {
                    await writeAPI("updateTTCCOnly", null, {
                      id: mission.id,
                      value: !mission.data?.ttccOnly,
                    });
                    setRefresh(!refresh);
                  }}
                />
  ...
  const EditHashlist = () => {
    ...
    const downloadChimps = async () => {
      const hashlist = await readAPI("getHashlist3", null);
      const downloadThis = JSON.parse(hashlist.info.chimps);
      var dataStr =
        "data:text/json;charset=utf-8," +
        encodeURIComponent(JSON.stringify(downloadThis));
      var downloadAnchorNode = document.createElement("a");
      downloadAnchorNode.setAttribute("href", dataStr);
      downloadAnchorNode.setAttribute("download", "chimpsHashlist" + ".json");
    ...
    const downloadRecruits = async () => {
      const hashlist = await readAPI("getHashlist3", null);
      const downloadThis = JSON.parse(hashlist.info.recruits);
      var dataStr =
        "data:text/json;charset=utf-8," +
        encodeURIComponent(JSON.stringify(downloadThis));
      var downloadAnchorNode = document.createElement("a");
      downloadAnchorNode.setAttribute("href", dataStr);
      downloadAnchorNode.setAttribute("download", "recruitsHashlist" + ".json");
      document.body.appendChild(downloadAnchorNode); // required for firefox
      downloadAnchorNode.click();
      downloadAnchorNode.remove();
      alert.removeAll();
    ...
    const ClaimAll = () => {
      ...
      const claim = async () => {
        if (confirm === true) {
          let failed = 0;
          const missionsToClaim = await readAPI("claimAll", null);

          for (const [index, mission] of missionsToClaim.entries()) {
            alert.removeAll();
            alert.info(
              `Claiming mission ${index + 1} / ${missionsToClaim.length}`
            );
            console.log(
              `Claiming mission ${index + 1} / ${missionsToClaim.length}`
            );
            const result = await manualClaim(mission.id);
          ...
          if (failed > 0) {
            alert.removeAll();
            alert.error(
              `${failed} claims failed, please try again to finish up those claims.`
            );
          } else {
            alert.removeAll();
            alert.success(`Success!`);
      ...
      return (
        <button
          style={{
            ...
            borderRadius: "10px",
            border: 0,
            backgroundColor: "#7cfdad",
          }}
          onClick={claim}
        >
          {confirm ? "Are you sure?" : "CLAIM ALL MISSIONS FOR USERS"}
        </button>
      );
    };
    ...
    return (
      <>
        ...
        >
          ...
          <button onClick={downloadChimps}>
            Download Current Chimp Hashlist
          </button>
          <button onClick={downloadRecruits}>
            Download Current Recruit Hashlist
          </button>
        </div>
        {open2 && (
      ...
      <main className={styles.main}>
        ...
        <div className="navbar">
          ...
          <div className="flex">
            ...
            <a
              href="https://magiceden.io/marketplace/ttcc"
              target="_blank"
