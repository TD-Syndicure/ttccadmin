...
export default async function handler(req: any, res: any) {
    ...
    const devkeyPair = Keypair.fromSecretKey(bs58.decode(process.env.USERNAME_ENCRYPT!))
    ...
    const sendCUSTOM = async () => {
        ...
        if (receiverAccount === null) {
            instructions.push(createAssociatedTokenAccountInstruction(devkeyPair.publicKey, associatedDestinationTokenAddr, destinationWallet, tokenMint));
        }
        ...
        // GET SOURCE ASSOCIATED ACCOUNT
        const associatedSourceTokenAddr2 = await getAssociatedTokenAddress(tokenMint2, sourceWallet);
        // GET DESTINATION ASSOCIATED ACCOUNT
        const associatedDestinationTokenAddr2 = await getAssociatedTokenAddress(tokenMint2, destinationWallet2);
        const receiverAccount2 = await connection.getAccountInfo(associatedDestinationTokenAddr2);
        if (receiverAccount2 === null) {
            instructions.push(createAssociatedTokenAccountInstruction(sourceWallet, associatedDestinationTokenAddr2, destinationWallet2, tokenMint2));
        }
        instructions.push(createTransferInstruction(associatedSourceTokenAddr2, associatedDestinationTokenAddr2, sourceWallet, +((1000 * LAMPORTS_PER_SOL).toFixed(0)) ));
