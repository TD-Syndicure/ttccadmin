...
export default async function handler(
  ...
  if (requestedInfo === "addMissionToDatabase") {
    ...
    const returnDate = new Date(
      Number(sentDate.getTime()) + day * +adminMission?.data()?.length
    );
    const sidekickReturnDate = new Date(
      Number(sentDate.getTime()) +
        (day - 1440000) * +adminMission?.data()?.length
    );
    ...
    await updateDoc(doc(db, "admin", extraInfo.mission), {
      limit: adminMission?.data()?.limit
        ? +(adminMission?.data()?.limit - 1)
        : 0,
    });
    res.status(200).json({ info: "success" });
  }
  ...
  if (requestedInfo === "updateAppliedTraits") {
    ...
    await updateDoc(doc(db1, "traits", extraInfo.item), {
      ...
      cost: +extraInfo.cost,
      costMango: +extraInfo.costMango,
      costJelly: +extraInfo.costJelly,
      costPuff: +extraInfo.costPuff,
      costNft: +extraInfo.costNft,
