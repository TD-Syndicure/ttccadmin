...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const getClaimsAvailable = async () => {
        let localArr: any = []
        const q = query(collection(db, "missions"), where("owner", "==", walletAddress), where("claimed", "==", false));
    ...
    const getProgress = async() => {
        const q = query(collection(db, "missions"), where("claimed", "==", false), where("mission", "==", extraInfo));
        const querySnapshot = await getDocs(q);
        return querySnapshot.docs.length
    }

    const checkIfUsersNFT = async() => {
        const q = query(collection(db, "missions"), where("claimed", "==", false), where("nftHash", "==", extraInfo));
        const querySnapshot = await getDocs(q);
        return querySnapshot.docs.length
    }
